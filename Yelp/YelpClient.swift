...
class YelpClient: BDBOAuth1RequestOperationManager {
    ...
    func searchWithTerm(_ term: String, sort: YelpSortMode?, categories: [String]?, deals: Bool?, completion: @escaping ([Restaurant]?, Error?) -> Void) -> AFHTTPRequestOperation {
        ...
        return self.get("search", parameters: parameters,
                        success: { (operation: AFHTTPRequestOperation, response: Any) -> Void in
                            if let response = response as? [String: Any]{
                                let dictionaries = response["restaurants"] as? [NSDictionary]
                                if dictionaries != nil {
